...
public:
    vector<int> pathsWithMaxScore(vector<string>& board) {
        ...
        // dp[i][j] = {best score, count of paths} to reach E from (i,j)
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                ...
                if (isValid(i - 1, j, board)) {              // move up
                    auto [score, paths] = dp[i - 1][j];       //solve(i-1, j)
                    upScore = score;
                    upPaths = paths;
                    if (upPaths > 0)
                ...
                if (isValid(i, j - 1, board)) {              // move left
                    auto [score, paths] = dp[i][j - 1];       //solve(i, j-1)
                    leftScore = score;
                    leftPaths = paths;
                    if (leftPaths > 0)
                ...
                if (isValid(i - 1, j - 1, board)) {          // move up-left
                    auto [score, paths] = dp[i - 1][j - 1];   //solve(i-1, j-1)
                    diagScore = score;
                    diagPaths = paths;
                    if (diagPaths > 0)
        ...
        auto result = dp[n - 1][n - 1]; //solve(n-1, n-1)
